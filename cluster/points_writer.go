...
type PointsWriter struct {
	...
	Subscriber interface {
		Points() chan<- *rpc.WritePointsRequest
	}
	subPoints chan<- *rpc.WritePointsRequest
...
func (w *PointsWriter) WritePoints(database, retentionPolicy string, consistencyLevel models.ConsistencyLevel, points []models.Point) error {
	...
	w.mu.RLock()
	select {
	case w.subPoints <- &rpc.WritePointsRequest{Database: database, RetentionPolicy: retentionPolicy, Points: points}:
		ok = true
	default:
	}
	w.mu.RUnlock()
